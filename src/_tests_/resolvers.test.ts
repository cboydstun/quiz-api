...
describe('Mutation resolvers', () => {
  describe('register', () => {
    ...
    it('should create a new user when input is valid', async () => {
      ...
      console.log('Test result:', result); // Add this line
      ...
      expect(authUtils.generateToken).toHaveBeenCalledWith(expect.objectContaining(mockUser));
      expect(mockUser.save).toHaveBeenCalled();
    });

    it('should throw UserInputError when username or email already exists', async () => {
      (User.findOne as jest.Mock).mockResolvedValue({ username: 'existinguser', email: 'existing@example.com' });

      await expect(
        resolvers.Mutation.register(
          null,
          { input: { username: 'existinguser', email: 'existing@example.com', password: 'password123' } },
          {} as any
        )
