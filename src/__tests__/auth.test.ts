...
describe('Authentication and Authorization', () => {
  describe('checkAuth', () => {
    it('should return user data if a valid token is provided', () => {
      const mockUser: DecodedUser = { _id: '123', email: 'test@example.com', role: 'USER' };
  ...
  describe('checkPermission', () => {
    it('should not throw an error if user has required permission', () => {
      const mockUser: DecodedUser = { _id: '123', email: 'admin@example.com', role: 'ADMIN' };
      expect(() => checkPermission(mockUser, ['ADMIN', 'SUPER_ADMIN'])).not.toThrow();
    });

    it('should throw ForbiddenError if user does not have required permission', () => {
      const mockUser: DecodedUser = { _id: '123', email: 'user@example.com', role: 'USER' };
      expect(() => checkPermission(mockUser, ['ADMIN', 'SUPER_ADMIN'])).toThrow(ForbiddenError);
    });

    it('should throw ForbiddenError if user object is not provided', () => {
      expect(() => checkPermission({} as DecodedUser, ['ADMIN', 'SUPER_ADMIN'])).toThrow(ForbiddenError);
    });
  });
